...
class ScatteringSimulator():
	...
	def __init__(self, linespec_dir, path):
		...
		-----------
		linespec_dir: str
			directory with only .VERT files from line spectrum
		path: str
			path to .DAT file with topography of line spectrum
		...
		self.scattering_model = ScatteringModel(self.atoms_g_nm)
	...
	def simulate_and_save_line_spectrum(self, n_spectra=5, n_bias=5):
		...
		Parameters:
		-----------
		n_bias: int, defaults to 5
			how many bias values in each spectrum should be used
		n_spectra: int, defaults to 5
			how many spectra in the line should be used
		...
		print("simulating line spectrum...")
		print("data is available at %d points and %d biases" %(len(self.lsp), len(self.biases)))
		print("for now I will only calculate at %d points and %d biases" %(n_spectra, n_bias))

		# sparsely sample the biases
		ebias = int(len(self.biases)/n_bias)
		biases = self.biases[::ebias]

		# sparsely sample the line spectrum points
		espectra = int(len(self.lsp)/n_spectra)
		lsp = self.lsp[::espectra]

		# use scattering_model.py to simulate spectrum at points
		self.ls = self.scattering_model.line_spectrum_at_points(lsp, biases)
		# self.ls = sm.line_spectrum_at_points(lsp, self.atoms_g_nm, )

		# convert to array and remove any nan values
		self.ls = np.array(self.ls)
		pdb.set_trace()
		self.d = self.ls[~np.isnan(self.ls).any(axis=1)]

		# make sure that we're actually plotting something
		assert(not np.all(np.all(np.isnan(self.ls), axis=0)))
