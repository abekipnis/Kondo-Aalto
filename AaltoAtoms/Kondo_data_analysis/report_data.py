import os, scipy, pickle, platform
import pandas as pd
...
from .analyze_data import analyze_data, get_old_Ag_Co_corrals, fit_and_plot_functional_curve
from .analyze_data import show_current_param_fit_result, plot_radial_width_dependence

from ..utils.visualizations import show_waterfall
from ..utils.interpolate_dIdV_v_r import imshow_dIdV_vs_r
from ..utils import labellines
from .analyze_data import basepath
from . import read_vertfile
...
def get_corralspec(datfile: str, l: list) -> list:
    ...
    def is_in(c: corralspectrum, file: str):
        ...
        return boolean
    ret = [c for c in l if is_in(c, datfile)][0]
    print(ret)
    return ret
...
def create_Ag_cache() -> list:
    """
        ...
        Returns:
    cache: list of [bias, dIdV, colors, radius, fit_bias, fit_dIdV]
    """
    directory = os.path.join(basepath,"04-11 Ag Co")
    wfall = {"A220411.133438.dat": "A220411.134351.L0013.VERT",
            ...
            "A220411.215004.dat": "A220411.215940.L0015.VERT", # used to be L0016
    ...
    for n, dat in enumerate(list(wfall.keys())):
        C = CircCorralData(os.path.join(directory, dat),"")
        C.occupied = True
        C.corral = True
        C.subtract_plane()
        ...
        S = Spec(os.path.join(directory,wfall[dat]))
        norm_val = S.dIdV[0]
...
def show_Ag_waterfall(cache: list, bias_idx: int=3, dIdV_idx: int =2) -> None:
    """
    Plot the data created by create_cache()

    Parameters:
        cache: list of [bias, dIdV + offset, color, radius, fit_bias, fit_dIdV]
            from create_Ag_cache() function
        bias_idx:
        dIdV_ idx:
...
def save_data(Co_Co_data_loc, Co_Ag_data_loc, e0_fixed_val: float = np.nan) -> None:
    """
    Analyze data defined by corralspectrum arrays in data_array
...
# following lines are for testing purposes, etc.
