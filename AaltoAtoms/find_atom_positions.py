...
# DAC values are in this range and map to -10V to 10V. we use this function to convert DAC values to z topography data
...
class CircCorralData:
    ...
    def get_region_centroids(self, diamond_size=2, sigmaclip=4, show=True, percentile=99, edge_cutoff=0.5):
        ...
        lim = x[np.argmin(abs(normed_cdf - percentile / 100.))]
        tf = np.array([self.im > lim])

        #diamond = morphology.diamond(diamond_size)
        # maxima = morphology.h_maxima(self.im, sigmaclip * np.std(self.im) + np.mean(self.im))
        #maxima = morphology.local_maxima(self.im, sigmaclip * np.std(self.im) + np.mean(self.im))
...
# all regions might be same size, in which case we're spot on
    ...
    def plot_circle_fit(self, points, radius, center, label, pix_or_nm="nm"):
        ...
        if pix_or_nm == "pix":
            ...
            vechl_min, vechl_max, vecvl_min, vecvl_max = list(
                map(self.pix_to_nm, [vechl_min, vechl_max, vecvl_min, vecvl_max]))
    ...
    def make_hex_lattice(self, theta, offseta=0, offsetb=0):
        ...
        for n in np.arange(-nrows / 2, nrows / 2 + 1, 1):
            for m in np.arange(-natoms_per_row / 2, natoms_per_row / 2 + 1, 1):
                if n % 2 == 0:
                    ls.append(array([n * self.nm_to_pix(d), m * self.nm_to_pix(b)]) + self.nm_to_pix(
                        array([np.sqrt(2) / 2. / np.sqrt(3) * a, 0])))
                else:
                    ls.append(array(
                        [n * self.nm_to_pix(d), (m * self.nm_to_pix(b) + self.nm_to_pix(b / 2))]) + self.nm_to_pix(
                        array([np.sqrt(2) / 2 / np.sqrt(3) * a, 0])))
    ...
    def fit_lattice(self, niter):
        ...
        result = least_squares(fix_self(self), init, bounds=bounds, verbose=2,
                               max_nfev=niter)  # s, method="dogbox", ftol=1e-11, xtol=1e-10)
...
class CircCorral(CircCorralData):
    def __init__(self, topo, zconst, nmx, nmy):
        # self.im = DAC_V_conv(topo) * zconst
        self.im = topo
        self.imshape = self.im.shape
        ...
        self.ang_ppx_x = nmx * 10 / self.xPix
        self.ang_ppx_y = nmy * 10 / self.yPix
...
def clockwiseangle(point, origin, refvec):
