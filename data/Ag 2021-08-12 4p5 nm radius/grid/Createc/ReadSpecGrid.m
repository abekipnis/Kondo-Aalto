...
header.nx =                         85;% fread(filename, 1,  'int32');
header.ny =                         85;%fread(filename, 1,  'int32');
...
header.xend =                       85;%fread(filename, 1,  'int32');
header.ystart =                     fread(filename, 1,  'int32');
header.yend =                       85; %fread(filename, 1,  'int32');
header.specgridchannelselectval2=   fread(filename, 1,  'int32');
header.specgridnx =                 85; %fread(filename, 1,  'int32');
header.specgridny =                 85; %fread(filename, 1,  'int32');
...
VZ =                                transpose(fread(filename, [header.specgridchan, header.vertpoints], 'single'));
V =                                 single(VZ(:,1));
Z =                                 single(VZ(:,2));
