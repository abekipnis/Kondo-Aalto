...
def animate_cube(cube_array, cut=True, mn=0, sd=0, interval=75, cmap='hot', ptx=0, pty=0, ptx2=0, pty2=0):
    ...
    offset = 10
    ax2.plot(specvz3[:,-1][offset:-1],cube_array[:,ptx, pty][offset:-1])
    ax2.plot(specvz3[:,-1][offset:-1],cube_array[:,ptx2, pty2][offset:-1])
    ...
    def updatefig(i):
        ...
        # ax2.remove()
        ax2.clear()
        ax2.plot(specvz3[:,0][offset:-1],cube_array[:,ptx, pty][offset:-1])
...
if __name__ == "__main__":
    ...
    try:
        ...
        specdata = data.reshape(a,b,len(specvz3),int(len(data)/a/b/len(specvz3)))
    # pdb.set_trace()
    animate_cube(specdata[:,:,:,1].T, False, ptx= 45, pty=44, ptx2=52, pty2=44)
